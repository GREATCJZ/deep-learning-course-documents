...
class Im2Latex(LightningDataModule):
    ...
    def __init__(self, batch_size: int = 8, num_workers: int = 0, pin_memory: bool = False,) -> None:
        super().__init__()
        ...
        # formula_file = self.data_dirname / "im2latex_formulas.norm.lst"
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage in ("fit", None):
            train_image_names, train_formulas = get_split(
                self.all_formulas, self.data_dirname / "im2latex_train_filter.lst",
            )
            ...
            val_image_names, val_formulas = get_split(
                self.all_formulas, self.data_dirname / "im2latex_validate_filter.lst",
            )
        ...
        if stage in ("test", None):
            test_image_names, test_formulas = get_split(
                self.all_formulas, self.data_dirname / "im2latex_test_filter.lst",
            )
